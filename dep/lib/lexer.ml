module Lexer = struct
  ...
  let new_context input =
    { source = input; start = -1; line = 0; current = 0; tokens = [] }

  let is_at_end context = context.current >= String.length context.source

  let is_digit character =
    match character with Some c when c >= '0' && c <= '9' -> true | _ -> false
  ...
  let advance_line context = { context with line = context.line + 1 }

  let advance_line_if_newline next_char context =
    if next_char = Some '\n' then advance_line context else context

  let current_char context =
    try Some (String.get context.source (context.current - 1))
    with Invalid_argument _ -> None

  let peek_char context =
    try Some (String.get context.source context.current)
    with Invalid_argument _ -> None
  ...
  let expect_char context expected_char =
    if is_at_end context then (false, context)
    else
  ...
  let is_next_char context expected_char =
    if is_at_end context then false
    else
      match peek_char context with
  ...
  let add_literal token_type literal_type context =
    let text =
      String.sub context.source context.start (context.current - context.start)
    in
  ...
  let add_keyword token_type context = add_literal token_type None context
  ...
  let add_conditional_token context possible_options token_fallback =
    try
      let _, matching_token_type =
        List.find
          (fun (possible_char, _) -> is_next_char context possible_char)
          possible_options
      in
      add_keyword matching_token_type (advance context)
    with Not_found -> add_keyword token_fallback context
  ...
  let rec add_string context =
    ...
    if (not is_done) && not is_error then
      context
      |> advance_line_if_newline (peek_char context)
      |> advance |> add_string
    else if is_error then failwith "Unterminated string."
    else
      let advanced_context = advance context in
      let string_value =
        String.sub context.source
          (advanced_context.start + 1)
          (advanced_context.current - advanced_context.start)
      in
      add_literal STRING (Some (STRING_LITERAL string_value)) advanced_context

  let rec scan_integer context =
    if is_digit (peek_char context) then context |> advance |> scan_integer
    else context
  ...
  let add_number context =
    ...
    let number_literal =
      String.sub advanced_context.source advanced_context.start
        (advanced_context.current - advanced_context.start)
    in
    let number_value = float_of_string number_literal in
  ...
  let scan_token context =
    ...
    | Some '\r' | Some '\t' | Some ' ' ->
        advanced_context (* ignore whitespace *)
    ...
    | Some '=' ->
        add_conditional_token advanced_context
          [ ('=', EQUALITY); ('>', THICK_ARROW) ]
          EQUAL
    | Some '>' -> add_conditional_token advanced_context [ ('=', GT_EQ) ] GT
    | Some '<' -> add_conditional_token advanced_context [ ('=', LT_EQ) ] LT
    | Some '!' ->
        add_conditional_token advanced_context [ ('=', NOT_EQUAL) ] EQUAL
    | Some ':' -> add_keyword COLON advanced_context
  ...
  let rec scan_all_tokens context =
    if not (is_at_end context) then context |> scan_token |> scan_all_tokens
    else context
